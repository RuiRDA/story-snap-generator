  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      if (canvasRef.current) {
        ...
        if (ctx) {
          canvasRef.current.width = STORY_CANVAS_DIMENSIONS.width;
          canvasRef.current.height = STORY_CANVAS_DIMENSIONS.height;
          ctx.drawImage(img, 0, 0, STORY_CANVAS_DIMENSIONS.width, STORY_CANVAS_DIMENSIONS.height);
    ...
    try {
      if (img) {
        ...
        if (ctx) {
          canvasRef.current.width = STORY_CANVAS_DIMENSIONS.width;
          canvasRef.current.height = STORY_CANVAS_DIMENSIONS.height;
          ctx.drawImage(overlayImage, 0, 0, STORY_CANVAS_DIMENSIONS.width, STORY_CANVAS_DIMENSIONS.height);
          setComposedImage(canvasRef.current.toDataURL("image/png"));
        }
      }
    } catch (error) {
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-center">
        ...
        >
          ...
          <canvas 
            ref={canvasRef}
            className="hidden"
            width={STORY_CANVAS_DIMENSIONS.width}
            height={STORY_CANVAS_DIMENSIONS.height}
          />
        ...
        <div className="w-full lg:flex-1 space-y-8">
          <div className="text-center lg:text-left animate-slide-up">
            ...
            <p className="text-muted-foreground text-sm sm:text-base md:px-0">
              Crie imagens para Instagram Stories posicionando a sua foto dentro do frame do evento Saia do Caixão.
            </p>
          </div>
